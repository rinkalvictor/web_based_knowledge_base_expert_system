...
def comment_thread(request, id):
    ...
    if form.is_valid() and request.user.is_authenticated():
        ...
        qs = Rule.objects.filter(post_id = obj_id)
        # return HttpResponse(qs.query)
        is_first = 0
        if qs.count() == 0:
            is_first = 1
        response_qs = Comment.objects.values_list('id', flat= True)
        response_id = response_qs.first()
        solution_id = int(request.POST.get("parent_id"))
        # return HttpResponse(solution_id)
        rule= Rule()
        rule.post_id = obj_id
        rule.is_first = is_first
        rule.solution_id = solution_id
        rule.response_id = response_id
        rule.save()
